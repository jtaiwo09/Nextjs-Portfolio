...
const ActionButton = ({ text, type, handleClick, loading }) => {
  return (
    <button
      type={type ? "button" : "submit"}
      onClick={handleClick}
      className="block text-hover-color group w-full h-[50px] max-w-[200px] leading-[50px] relative mt-5 animate-[fadeInAnimation_1s_1.8s_backwards] whitespace-nowrap"
    >
      <div className="w-full h-full relative overflow-hidden rounded-[0_15px_0_15px]">
        ...
        {loading ? (
          <ImSpinner2 className="text-hover-color text-[20px] text-center absolute left-0 right-0 top-0 bottom-0 m-auto animate-spin" />
        ) : (
